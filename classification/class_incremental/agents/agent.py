...
from utils.metric import accumulate_acc, accumulate_seen_acc, AverageMeter, Timer
...
class Agent(nn.Module):
    ...
    def train_model(self, train_loader, val_loader=None):
        ...
        for epoch in range(self.config['schedule'][-1]):
            ...
            losses, reg_losses = AverageMeter(), AverageMeter() 
            acc = AverageMeter()
            ...
            data_timer.tic()
            batch_timer.tic()
            self.log('Itr\t\tTime\t\t  Data\t\t  Loss\t\t RegLoss\t\t Acc') 
            ...
            for i, (inputs, target, task) in enumerate(train_loader):
                ...
                loss, reg_loss = self.criterion(output, target, task)
                ...
                reg_losses.update(reg_loss, inputs.size(0))
                ...
                if ((self.config['print_freq'] > 0) and (i % self.config['print_freq'] == 0)) or (i + 1) == len(
                    ...
                    self.log('[{0}/{1}]\t'
                             ...
                             '{reg_loss.val:.3f} ({reg_loss.avg:.3f})\t'
                             '{acc.val:.2f} ({acc.avg:.2f})'.format(
                        i, len(train_loader), batch_time=batch_time,
                        data_time=data_time, loss=losses, reg_loss=reg_losses, acc=acc)) 
    ...
    def validation(self,dataloader, val_name=None):
        ...
        for i, (inputs, target, task) in enumerate(dataloader):
            ...
            if self.multihead:
                ...
                val_acc = accumulate_seen_acc(output['All'][:, :self.valid_out_dim], target, val_acc)
    ...
    def cross_entropy(self, preds, targets, tasks):
        ...
        if self.multihead:
            ...
            loss = self.criterion_fn(pred / self.config['temp'], targets)
        return loss
