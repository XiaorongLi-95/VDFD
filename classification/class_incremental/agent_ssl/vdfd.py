...
class VDFD(Agent):
    ...
    def svd(self, imp, sigv):
        left_eigen_vec, eigen_val, right_eigen_vec = torch.svd_lowrank(torch.stack(imp).t(), q=sigv)
    ...
    def com_prototype(self):
        ...
        for t in range(self.task_count):
            if self.multihead:
                ...
                end = start + self.config['task_out_space'][str(t+1)] * 4 # SSL
                weight = self.model.last['All'].weight[start:end,:].detach()
                start = end
            normalized = f.normalize(weight[::4], p=2, dim=1) # SSL
